...
class DateKoans extends Koans {
	...
	void testParseStringToDate() {
		...
		assert Date.class.name == date.class.name
		assert 101 == date.year
		assert 8 == date.month
		assert 11 == date.date
		assert 00 == date.hours
		assert 00 == date.minutes
	}
	...
	void testCreateDateViaConstructor() {
		...
		assert Date.class.name == date2.class.name
	...
	void testCreateDateFromCalendar() {
		...
		assert GregorianCalendar.class.name == cal.class.name
		assert Date.class.name == cal.time.class.name
		def date = cal.time
		assert 101 == date.year
		assert 8 == date.month
		assert 11 == date.date
	...
	void testDateMath() {
		...
		assert 00 == datePlusOne.hours
		assert 00 == datePlusOne.minutes
		assert 00 == datePlusOne.seconds
