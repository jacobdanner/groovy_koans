...
class StringKoans extends Koans {
	...
	void testDoubleQuoteStringType() {
		assertEquals String.class.name, "Any ole String".class.name
	}
	
	void testSingleQuoteStringType() {
		assertEquals String.class.name, 'A Single Quote String'.class.name
	}
	
	void testSlashyQuoteStringType() {
		assertEquals String.class.name, /A Slashy String/.class.name
	}

	void testMultiLineStringType() {
		assertEquals String.class.name, """A Triple Quote String""".class.name
	}
	...
	void testInterpolatedStringType() {
		def answer = "c"
		assertEquals GStringImpl.class.name, "ab${answer}".class.name
		assertEquals String.class.name, 'ab${answer}'.class.name
		assertEquals GStringImpl.class.name, /ab${answer}/.class.name
	}
